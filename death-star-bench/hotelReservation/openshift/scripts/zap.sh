...
for s in consul frontend geo jaeger memcached-profile memcached-rate memcached-reserve mongodb-geo mongodb-profile mongodb-rate mongodb-recommendation mongodb-reservation mongodb-user profile rate recommendation reservation search user; do
...
for i in geo profile rate recommendation reservation user; do
  oc delete pv/$i -n ${NS} &
done
wait

for c in configmap-config-json; do
  oc delete cm/${c} -n ${NS}
done
